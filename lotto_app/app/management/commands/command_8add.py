...
class Command(BaseCommand):
    ...
    def _get_info_evaluate_future_game(self, options_last_games):
        return {
            ...
            'options_last_games': options_last_games,
        }

    def _update_info_evaluate_future_game(self):
        self.info_evaluate_future_game.update({'update': {}})
    ...
    def evaluate_future_game(self):
        ...
        if self._get_forbidden_circulation():
            ...
            return False
        options_last_games = [self.gen_probability._get_combination_options_8_add(_obj.numbers)
                              for _obj in self.gen_probability.game_objs[0:20]]
        ...
        self.info_evaluate_future_game = self._get_info_evaluate_future_game(options_last_games)
        print(self.print_info(self.info_evaluate_future_game))

        if self.exclude_one_numbers and self.exclude_three_numbers:
            return True
        if self.exclude_two_numbers and (self.exclude_one_numbers or self.exclude_three_numbers) and (
                options_last_games.count(COMBINATION_OPTIONS_8_ADD[self.gen_option]) <= 3):
            return True
        if self.exclude_two_numbers and self.preferred_added_number:
            return True
    ...
    def _comparison_combination_options(self, future_game_numbers):
        ...
        if sort_numbers == COMBINATION_OPTIONS_8_ADD[self.gen_option]:
            return False
        return True
    ...
    def _comparison_comparison_all_game_numbers(self, future_game_numbers):
        for _obj in self.gen_probability.game_objs:
    ...
    def pc_choice_numbers(self):
        ...
        for i in range(3000):
            ...
            if self._comparison_exclude_three_numbers(future_game_numbers, self.exclude_three_numbers):
                continue
            if self._comparison_comparison_all_game_numbers(future_game_numbers):
                continue
            break
